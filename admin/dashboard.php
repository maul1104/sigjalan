...
// Query untuk mengecek pengajuan baru dengan status 'pending' atau status lainnya yang sesuai
$query_pengajuan_baru = "SELECT COUNT(*) AS jumlah_pengajuan_baru FROM pengajuan WHERE status = 'Menunggu'";
$result_pengajuan_baru = mysqli_query($conn, $query_pengajuan_baru);
$data_pengajuan_baru = mysqli_fetch_assoc($result_pengajuan_baru);

// Jika ada pengajuan baru, tampilkan notifikasi SweetAlert
if ($data_pengajuan_baru['jumlah_pengajuan_baru'] > 0) {
    echo "
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                title: 'Pengajuan Baru!',
                text: 'Ada pengajuan baru dari masyarakat. Klik OK untuk melihat.',
                icon: 'info',
                confirmButtonText: 'OK'
            }).then((result) => {
                if (result.isConfirmed) {
                    window.location.href = 'pengajuan_masuk.php';
                }
            });
        });
    </script>
